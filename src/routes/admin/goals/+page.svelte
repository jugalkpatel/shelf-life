...
<div class="space-y-8">
	...
	<section class="grid gap-5 md:grid-cols-3">
		<SurfaceCard title="Readers" description="With a Shelf account">
			<p class="font-display text-4xl text-[var(--color-ink)]">{data.summary.totalReaders}</p>
		</SurfaceCard>
		<SurfaceCard title="Readers with goals" description="Have set a target">
			<p class="font-display text-4xl text-[var(--color-ink)]">{data.summary.readersWithGoal}</p>
		</SurfaceCard>
		<SurfaceCard title="Finished this year" description="Total books">
			<p class="font-display text-4xl text-[var(--color-ink)]">{data.summary.totalFinished}</p>
		</SurfaceCard>
	</section>

	<section class="space-y-4">
		<h2 class="font-display text-3xl text-[var(--color-ink)]">Per-reader progress</h2>
		{#if data.summary.perReader.length === 0}
			<p class="text-sm text-[var(--color-muted)]">No readers yet.</p>
		{:else}
				...
				<tbody>
					{#each data.summary.perReader as reader (reader.userId)}
						<tr class="border-b border-[var(--color-border)]">
							<td class="py-2 pr-4 text-[var(--color-ink)]">{reader.name}</td>
							<td class="py-2 pr-4 text-[var(--color-ink)]">{reader.targetBooks ?? '—'}</td>
							<td class="py-2 pr-4 text-[var(--color-ink)]">{reader.finishedBooks}</td>
							<td class="py-2 pr-4 text-[var(--color-ink)]">{reader.percentage}%</td>
						</tr>
					{/each}
				</tbody>
			</table>
